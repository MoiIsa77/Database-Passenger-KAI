...
conn = st.connection(
    "postgresql",
    type="sql",
    url="postgresql://modesign111:HmiD28WILsto@ep-muddy-limit-08504363.ap-southeast-1.aws.neon.tech/Project3MoIsa",
)
with conn.session as session:
    query = text(
        "CREATE TABLE IF NOT EXISTS SCHEDULE (id serial, doctor_name varchar, patient_name varchar, gender char(25), \
    ...
    for _, result in data.iterrows():
        ...
        with st.expander(f"a.n. {patient_name_lama}"):
            with st.form(f"data-{id}"):
                doctor_name_baru = st.selectbox(
                    "doctor_name", list_doctor, list_doctor.index(doctor_name_lama)
                )
                patient_name_baru = st.text_input("patient_name", patient_name_lama)
                gender_baru = st.selectbox(
                    "gender", list_symptom, list_symptom.index(gender_lama)
                )
                symptom_baru = st.multiselect(
                    "symptom",
                    ["cough", "flu", "headache", "stomache"],
                    eval(symptom_lama),
                )
